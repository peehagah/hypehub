/**
 * Seed script: inserts agents into Supabase agents table
 * Run with: npx ts-node --project tsconfig.json scripts/seed-agents.ts
 */
...
async function getWorkspaceId(slug: string): Promise<string | null> {
  const { data, error } = await supabase
    ...
    .single()
  if (error) {
    console.error(`Workspace not found: ${slug}`, error.message)
    return null
  }
  return data.id
}
...
async function seed() {
  ...
  for (const agentDef of agentsToSeed) {
    ...
    if (!workspaceId) {
      console.log(`  Skipping agents for ${workspaceSlug} (workspace not found)`)
      continue
    }
    ...
    // Insert agent using actual DB columns
    ...
    if (error) {
      console.error(`  ERROR inserting ${agentData.name}:`, error.message)
    } else {
      console.log(`  Inserted: ${agentData.name} (${agentData.type}) -> workspace: ${workspaceSlug}`)
    }
  }
