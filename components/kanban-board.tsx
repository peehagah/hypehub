...
export function KanbanBoard({ tasks, workspaces }: KanbanBoardProps) {
  ...
  return (
    <div>
      {/* Mobile column tabs */}
      <div className="flex md:hidden mb-4 rounded-xl border border-[#2a2d3e] overflow-hidden">
        {COLUMNS.map((col) => {
          ...
          return (
            <button
              ...
              className={cn(
                'flex-1 flex items-center justify-center gap-1.5 py-3 text-xs font-semibold transition-colors min-h-[44px]',
                isActive ? 'bg-[#2a2d3e] text-white' : 'bg-[#1a1d2e] text-slate-500'
              )}
            >
              <span style={{ color: isActive ? col.color : undefined }}>{col.icon}</span>
      ...
      <div className="md:hidden">
        {COLUMNS.filter((c) => c.id === mobileCol).map(renderColumn)}
      </div>
    </div>
  )
}
